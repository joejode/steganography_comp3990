...
void encodeMessage(int vqVals[MAX][MAX],int);
void decodeMessage(int);
void getBin(int num, int bits, char *binVal);
int getDec(int *, int);
void printStats(int sIndex, int bitcount, int c1, int c2, int c3, int c4, int m);
void checkSecret();
void checkIndex();
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for (m = 4; m <= 4; m++)
	{
		memset(index, 0, sizeof(index[0][0]) * MAX * MAX);
...
void encodeMessage(int vqVals[MAX][MAX],int M)
{
	char bVal[10];
	int  x, u, l, t, g, len = 0, seedCnt = 0, secretInx = 0;
	double scount = 0.0, bitcount = 0.0, d;
	char b1, b2;
	int c1 = 0, c2 = 0, c3 = 0, c4 = 0;

	int codeLen = strlen(codeStream);
	memset(codeStream, '\0',strlen(codeStream));
	...
	sIndex = secretInx;
	printStats(sIndex, strlen(codeStream), c1, c2, c3, c4, M);
}
